...
A <- ggplot(data = fulldf, aes(Frame, N)) +
	...
	geom_vline(data = data.frame(x = c(det_food_times, sto_food_times, NA, NA), 
				     cond = c('DET', 'DET', 'STO', 'STO', 'No food', 'No food')),
	...
	scale_y_continuous('Activity (number of ants)',
			   ...
			   sec.axis = sec_axis(trans = ~.*1,
			   		    ...
			   		    name = 'Interactions (cumulative sum)'))+
	theme(legend.position = c(0.9, 0.7),
	      legend.background = element_rect(fill = 'white', linewidth = 1.5,
	      				 linetype = 'solid', color = 'black'),
	      legend.title = element_blank(),
	      strip.text = element_text(size = 18, margin = margin(b = 5, t = 5)),
	      aspect.ratio = 0.75)
...
nf_phases <- list(p1 = vector('list', length(nf)),
		   p2 = vector('list', length(nf)),
		   p3 = vector('list', length(nf)))
...
# Sys.time() - t0
# beepr::beep(3)
t <- round(apply(do.call('rbind', lapply(seq_along(det), function(i){
	range(do.call('rbind', det[[i]]@food)$t)
	})), 2, mean))
for(i in seq_along(nf)){
	nf_phases$p1[[i]] <- mutual_info(nf[[i]], t = 1:t[1], edges)
	nf_phases$p2[[i]] <- mutual_info(nf[[i]], t = (t[1]+1):t[2], edges)
	nf_phases$p3[[i]] <- mutual_info(nf[[i]], t = (t[2]+1):21600, edges)
}
...
znf <- list(z1 = apply(do.call('rbind', nf_phases$p1), 2, mean),
	     z2 = apply(do.call('rbind', nf_phases$p2), 2, mean),
	     z3 = apply(do.call('rbind', nf_phases$p3), 2, mean))
...
supervec <- rank(c(unlist(zdet), unlist(zsto), unlist(znf), recursive = T))
ztotal <- list(d1 = supervec[1:(length(supervec)/9)],
	       d2 = supervec[(length(supervec)/9 + 1):(2*length(supervec)/9)],
	       d3 = supervec[(2*length(supervec)/9 + 1):(3*length(supervec)/9)],
	       s1 = supervec[(3*length(supervec)/9 + 1):(4*length(supervec)/9)],
	       s2 = supervec[(4*length(supervec)/9 + 1):(5*length(supervec)/9)],
	       s3 = supervec[(5*length(supervec)/9 + 1):(6*length(supervec)/9)],
	       n1 = supervec[(6*length(supervec)/9 + 1):(7*length(supervec)/9)],
	       n2 = supervec[(7*length(supervec)/9 + 1):(8*length(supervec)/9)],
	       n3 = supervec[(8*length(supervec)/9 + 1):(length(supervec))])
...
ztotaldt <- rbindlist(list(data.table(mi = supervec[1:(length(supervec)/9)], exptype = 'DET', phase = 'Exploration'),
			   data.table(mi = supervec[(length(supervec)/9 + 1):(2*length(supervec)/9)],
			   	   exptype = 'DET', phase = 'Exploitation'),
			   data.table(mi = supervec[(2*length(supervec)/9 + 1):(3*length(supervec)/9)],
			   	   exptype = 'DET', phase = 'Post-exploitation'),
			   data.table(mi = supervec[(3*length(supervec)/9 + 1):(4*length(supervec)/9)],
			   	   exptype = 'STO', phase = 'Exploration'),
			   data.table(mi = supervec[(4*length(supervec)/9 + 1):(5*length(supervec)/9)],
			   	   exptype = 'STO', phase = 'Exploitation'),
			   data.table(mi = supervec[(5*length(supervec)/9 + 1):(6*length(supervec)/9)],
			   	   exptype = 'STO', phase = 'Post-exploitation'),
			   data.table(mi = supervec[(6*length(supervec)/9 + 1):(6*length(supervec)/9)],
			   	   exptype = 'STO', phase = 'Exploration'),
			   data.table(mi = supervec[(7*length(supervec)/9 + 1):(6*length(supervec)/9)],
			   	   exptype = 'STO', phase = 'Exploitation'),
			   data.table(mi = supervec[(8*length(supervec)/9 + 1):(length(supervec))],
			   	   exptype = 'No food', phase = 'Post-exploitation')))
...
# 	   		   aes(x, y), shape = 24, fill = 'mediumpurple1', size = 3)
# )
lgnd <- get_legend(draw_hexagons(z = apply(rbind(do.call('rbind', zdet), do.call('rbind', zsto), do.call('rbind', znf)), 2, max),
			 size = 3, lineend = 'round',
			 add = draw_hexagons(size = 3.1, color = 'black', lineend = 'round', 
			 		    add = geom_foodpatches(
			 		    	food = det[[1]]@food, fill = 'mediumpurple1', color = 'white')
	   	...
	   	theme(aspect.ratio = 0.5, legend.position = 'bottom',
	   	      ...
	   	      plot.margin = margin(l = 5, r = -25, unit = 'pt'))+
	   	scico::scale_color_scico('Mutual information', palette = 'bilbao', direction = 1)+
	   	geom_point(data = data.frame(x = hex$x[hex$node == 634],
	   				     y = hex$y[hex$node == 634] - 35),
	   		   aes(x, y), shape = 24, fill = 'mediumpurple1', size = 3))
L2 <- list(draw_hexagons(z = ztotal$d1,
	   ...
	   draw_hexagons(z = ztotal$s1, size = 3, lineend = 'round',
	   	...
	   	geom_point(data = data.frame(x = hex$x[hex$node == 634],
	   				     y = hex$y[hex$node == 634] - 35),
	   		   aes(x, y), shape = 24, fill = 'mediumpurple1', size = 3),
	   draw_hexagons(z = ztotal$n1, size = 3, lineend = 'round',
	   	      add = draw_hexagons(size = 3.1, color = 'black', lineend = 'round')) +
	   	theme(aspect.ratio = 0.5, legend.position = 'none',
	   	      plot.margin = margin(l = -130, unit = 'pt')) +
	   	scico::scale_color_scico(palette = 'bilbao')+
	   ...
	   draw_hexagons(z = ztotal$n2, size = 3, lineend = 'round',
	   	      add = draw_hexagons(size = 3.1, color = 'black', lineend = 'round')) +
	   	theme(aspect.ratio = 0.5, legend.position = 'none',
	   	      plot.margin = margin(l = -130, unit = 'pt')) +
	   	scico::scale_color_scico(palette = 'bilbao')+
	   ...
	   draw_hexagons(z = ztotal$n3, size = 3, lineend = 'round',
	   	      add = draw_hexagons(size = 3.1, color = 'black', lineend = 'round')) +
	   	theme(aspect.ratio = 0.5, legend.position = 'none',
	   	      plot.margin = margin(l = -130, unit = 'pt')) +
	   	scico::scale_color_scico(palette = 'bilbao')+
...
# png('~/research/papers/SPIN_GLASSES/Figs_spinGlasses/Ndynamics_MI.png', width = 8000, height = 6000, res = 400)
# ggarrange(plotlist = list(A, grid.arrange(grobs = L2, nrow = 3, ncol = 3, as.table = FALSE)), 
# 	  nrow = 2, heights = c(0.38, 1-0.38))
# dev.off()

png('~/research/papers/SPIN_GLASSES/Figs_spinGlasses/Ndynamics_MI.png', width = 8000, height = 6000, res = 400)
ggarrange(plotlist = list(A, ggarrange(plotlist = L2, legend.grob = lgnd,
