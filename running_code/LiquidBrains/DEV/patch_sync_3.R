...
path <- '~/research/gits/AutomatAnts/results/2024/LR_nfd_rho/'
...
files <- f[grepl('position', f) & grepl('rho_0.001', f)]
# files <- f[grepl('position', f) & grepl('rho_1.001', f)]
pos_data <- data.table(read_parquet(paste0(path, files[1])))
for(i in files[2:length(files)]){
	pos_data[['z']] <- pos_data[['z']] + data.table(read_parquet(paste0(path, i)))[['z']]
}
