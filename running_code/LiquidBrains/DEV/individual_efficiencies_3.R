...
path <- '/home/polfer/research/gits/AutomatAnts/results/rec_noRec_nf/noRec/gaussian/'
files <- list.files(path)
files <- unique(unlist(regmatches(files, gregexpr('gaussian_\\d{1,2}', files))))
...
path2 <- '/home/polfer/research/gits/AutomatAnts/results/rec_noRec_nf/rec/uniform/'
files2 <- list.files(path2)
files2 <- unique(unlist(regmatches(files2, gregexpr('uniform_\\d{1,2}', files2))))

t0 <- Sys.time()
result_list_2 <- lapply(seq_along(files2), function(x){
	do.call('gc', args = list(verbose = FALSE))
	eff(path2, files2[x])
})
Sys.time()- t0
names(result_list_2) <- files2
L2 <- result_list_2[lapply(result_list_2,length) > 1]
result_dt_2 <- rbindlist(L2, idcol = TRUE)
colnames(result_dt_2)[1] <- 'condition'
result_dt_2[['condition']] <- 'sim_rec'


ggplot(data = melt(rbindlist(list(result_dt, result_dt_2, tps)), id.vars = 'condition'), 
