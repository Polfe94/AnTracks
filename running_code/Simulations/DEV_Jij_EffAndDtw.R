...
png('~/research/gits/AnTracks/plots/homoJij_dtw.png', height = 8000, width = 8000, res = 450)
klstrs
dev.off()
...
dtw <- rbindlist(list(dJij0, dJij0.4, dJij1, dJij1.5), idcol = T)

labeller <- function(x, pattern = 'theta_'){
	# sprintf(r'($\Theta = 10^{-%d}$)', as.numeric(gsub(pattern, '', x)))
	sprintf(r'($10^{-%d}$)', as.numeric(gsub(pattern, '', x)))
}




png('~/research/gits/AnTracks/plots/homoJij_efficiency.png', width = 4000, height = 2000, res = 200)
...
ggplot(data = global_eff_mlt, aes(factor(label, 
	...
	theme(strip.text = element_text(size = 16, margin = margin(t = 5, b = 5)))+
